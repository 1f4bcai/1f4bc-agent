...
describe('published package contract', () => {
  it('publishes a documented, typed public package without source or tests', async () => {
    ...
    expect(rootManifest.version).toBe('0.1.5')
    expect(lock.version).toBe('0.1.5')
    expect(lock.packages?.['']?.version).toBe('0.1.5')
    expect(lock.packages?.['packages/agent-cli']?.version).toBe('0.1.5')
    ...
    expect(readme).toContain('Version `0.1.4` was staged but rejected before registry publication. Its protected source tag remains an immutable audit record, but the package version is permanently retired and must not be approved or re-staged. Use `0.1.5` or later.')
    expect(readme).not.toContain('funded payment rails remain closed')
    ...
    expect(readme).toContain('one exact protected release tag currently being staged')
    expect(readme).toContain('never leave the broader `agent-v*` wildcard enabled')
